 ...
 */
L.Control.DynamicElevation = L.Control.extend({
  ...
  addData: function(d, layer) {
    ...
    if (layer) {
      if (layer._path) {
        L.DomUtil.addClass(
          layer._path,
          "elevation-polyline " + this.options.theme
        );
      }
      layer
  ...
  initialize: function(options) {
    ...
    // Aliases.
    if (typeof options.detachedView !== "undefined")
      this.options.detached = options.detachedView;
    if (typeof options.responsiveView !== "undefined")
      this.options.responsive = options.responsiveView;
    // if (typeof options.showTrackInfo !== "undefined")
    //   this.options.summary = options.showTrackInfo;
    // if (typeof options.summaryType !== "undefined")
    //   this.options.summary = options.summaryType;
    // if (typeof options.autohidePositionMarker !== "undefined")
      this.options.autohideMarker = options.autohidePositionMarker;
    if (typeof options.followPositionMarker !== "undefined")
      this.options.followMarker = options.followPositionMarker;
    if (typeof options.useLeafletMarker !== "undefined")
      this.options.marker = options.useLeafletMarker
        ? "position-marker"
        : "elevation-line";
    if (typeof options.leafletMarkerIcon !== "undefined")
      this.options.markerIcon = options.leafletMarkerIcon;
    // if (typeof options.download !== "undefined")
    //   this.options.downloadLink = options.download;
  ...
  loadDefer: function(data, opts) {
    ...
    if (document.readyState !== "complete")
      window.addEventListener("load", L.bind(this.loadData, this, data, opts), {
        once: true
      });
  ...
  loadGPX: function(data) {
    ...
    if (typeof L.GPX !== "function" && this.options.lazyLoadJS) {
      L.Control.DynamicElevation._gpxLazyLoader = this._lazyLoadJS(
        "https://cdnjs.cloudflare.com/ajax/libs/leaflet-gpx/1.4.0/gpx.js",
        L.Control.DynamicElevation._gpxLazyLoader
      );
      L.Control.DynamicElevation._gpxLazyLoader.then(callback);
    } else {
      callback.call();
    }
  },
  ...
  onAdd: function(map) {
    ...
    if (typeof d3 !== "object" && this.options.lazyLoadJS) {
      L.Control.DynamicElevation._d3LazyLoader = this._lazyLoadJS(
        "https://unpkg.com/d3@4.13.0/build/d3.min.js",
        L.Control.DynamicElevation._d3LazyLoader
      );
      L.Control.DynamicElevation._d3LazyLoader.then(callback);
    } else {
      callback.call();
    }
    return container;
  ...
  _appendFocusRect: function(g) {
    var focusRect = (this._focusRect = g
      .append("rect")
  ...
  _appendLegend: function(g) {
    ...
    var legend = (this._legend = g.append("g").attr("class", "legend"));

    var altitude = (this._altitudeLegend = this._legend
      .append("g")
  ...
  _applyData: function() {
    ...
    if (this.fire) this.fire("data_loaded",);
    if (this._map) this._map.fire("data_loaded");
  },
  ...
  _drawDragRectangle: function() {
    ...
    var dragEndCoords = (this._dragCurrentCoords = d3.mouse(
  ...
  _initChart: function() {
    ...
    if (opts.responsive) {
      if (opts.detached) {
        ...
        opts.height = offHe - 1 > 0 ? offHe - 1 : opts.height; // 20 = horizontal scrollbar size.
      } else {
        opts._maxWidth =
          opts._maxWidth > opts.width ? opts._maxWidth : opts.width;
        var containerWidth = this._map._container.clientWidth;
        opts.width =
          opts._maxWidth > containerWidth ? containerWidth - 30 : opts.width;
      }
    }
  ...
  _isXMLDoc: function(doc, lazy) {
    ...
    if (typeof doc === "string" && lazy) {
      ...
      var documentElement = (doc ? doc.ownerDocument || doc : 0)
        .documentElement;
  ...
  _showDiagramIndicator: function(item, xCoordinate) {
    ...
    //hide dist and alt on the graph
    var opts = this.options;
...
///////////////////////////////////////////////////////////////
  ...
  //             new CustomEvent("eletrack_download", { detail: evt })
  //           );
  ...
  _runners: [],
  getRunner(id) {
    return this._runners.find(r => r.id === id)
  },
  setRunner: function(data) { 
    let runner = this.getRunner(data.id);
    if (!runner) {
      runner = this.addRunner(data)
    }
    let x = this._x(data.dist);
    runner.point
      .selectAll("circle")
      .attr("cx", x)
      .attr("cy", this._y(this._data[this._findItemForX(x)].z))

      runner.point.selectAll('text')
        .attr("x", x)
        .attr("y", this._y(this._data[this._findItemForX(x)].z) + 3);
  },
  
  showRunner: function(id) {
    let runner = this.getRunner(id);
    if (runner) {
      runner.point.style("display", "block");
      runner.display = "block";
    }
  },

  hideRunner: function(id) {
    let runner = this.getRunner(id);
    if (runner) {
      runner.point.style("display", "none");
      runner.display = "none";
    }
  },

  selectRunner: function(id) {
    let runner = this.getRunner(id);
    if (runner) {
      runner.point.moveToFront();
      runner.point.select("circle")
        .style("stroke", "white")
  ...
  addRunner: function(data) { 
    //const point = this._g.append("g").attr("class", "runner");
    const point = this._g.append("g").attr("class", "point");
    let x = this._x(data.dist);
    const y = this._y(this._data[this._findItemForX(x)].z);

    point
      .style("display", data.display || "block")
      .append("svg:circle")
    ...
    if (data.onClick) {
      point.on('click', data.onClick);
      point.style('cursor', 'pointer');
    }

    if (data.onMouseOver) {
      point.on('mouseover', data.onMouseOver);
      point.style('cursor', 'pointer');
    ...
    if (data.onMouseMove) {
      ...
      point.on('mousemove', data.onMouseMove);    
      point.moveToFront(); 
      });
    }
    ...
    if (data.onMouseOut) {
      point.on('mouseout', data.onMouseOut);
      point.style('cursor', 'pointer');
    }

    const runner = { ...data, point }
  ...
  addCheckpoint: function(checkpoint) {
    let check = this._g.append("g").attr("class", "checkpoints");
    const item = this._findItemForLatLng(checkpoint.getLatLng())
    let x = this._x(item.dist);
    let y = this._y(item.z);
    check
      .append("svg:line")
      .attr("x2", x)
      .attr("y2", y)
    ...
    let tooltip = check.append('text')
      .attr("x", x)
      .attr("y", y)
      .attr("dy", "-.95em")         // set offset y position
      ...
      .text(function(d) {
                        var returnText;
                          if (d === checkpoint.feature.properties.controlpoint) { returnText = checkpoint.feature.properties.controlpoint;
                          }
                          if (d === checkpoint.feature.properties.name) { returnText = checkpoint.feature.properties.name;
                          }
                          return returnText;
                          })
    ...
    var mousemove = function(d) {
      ...
      .html(function() {
        var returnText;
          if (checkpoint.feature.properties.controlpoint) { 
            returnText = checkpoint.feature.properties.controlpoint;
          } 
          if (checkpoint.feature.properties.name) { 
            returnText = checkpoint.feature.properties.name;
          } 
          return returnText;
      })
    ...
    check
      ...
      .style("fill", function(d) {
        var returnColor;
        if (d === checkpoint.feature.properties.controlpoint) { returnColor = "black";
        } 
        if (d === checkpoint.feature.properties.name) { returnColor = "darkorange";
        }
        return returnColor;
      })
      // .style("fill", "#69b3a2")
...
L.control.dynamicElevation = function(options) {
  return new L.Control.DynamicElevation(options);
};
